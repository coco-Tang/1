  ...
  <div class="importVoiceDialog">
    ...
    >
      <el-form
        ...
        class="demo-ruleForm"
      >
        <el-form-item label="语种" prop="language" :class="{'hide':this.$route.name==='importList'}">
          <el-select
            style="width: 190px; margin-right: 30px"
            :disabled="disabled"
            v-model="ruleForm.recognize"
            placeholder="请选择"
          >
            <el-option
        ...
        <el-form-item
          ...
          label="处理内容"
          prop="langname"
          :class="{'hide':this.$route.name==='importList'}"
        >
          <el-checkbox-group @change="checkboxChange" v-model="ruleForm.checkList">
            <el-checkbox
              v-for="item in processBitsOpts"
              :label="item.value"
              :key="item.value"
        ...
        <el-form-item label="备注" prop="remark" :class="{'hide':this.$route.name==='importList'}">
          <el-input v-model="ruleForm.remark"></el-input>
        </el-form-item>

        <el-form-item label="语音格式" prop="format" v-if="!updateMsg.isEdit">
        ...
        <el-form-item label="上传语音" v-if="!updateMsg.isEdit">
        ...
        <div
          ...
          :class="rowData.length?'':'hide'"
          v-if="!updateMsg.isEdit"
        >
          <ag-grid-vue
      ...
      <span slot="footer" class="dialog-footer">
        <el-button
          v-if="this.$route.name==='importList'"
          type="primary"
          class="checked"
          @click="appendFile"
...
import { LANG, PROCESS_BITS, FMT } from "@/constant";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        // format: [
        //     { required: true, message: '请选择待上传的语音格式', trigger: ['blur', 'change'] }
        // ]
      },
  ...
  methods: {
    ...
    showDialog() {
      ...
      if (this.updateMsg.isEdit) {
        ...
        if (
          this.updateMsg.data.language != "英语" &&
          this.updateMsg.data.language != "汉语" &&
          this.updateMsg.data.language != "维语"
        ) {
        ...
        this.ruleForm.checkList = this.calcProcessFlag(
          this.updateMsg.data.recogflag
        );
      }
    },
    callInputChange() {
    ...
    uploadFile() {
      ...
      if (!this.updateMsg.isEdit) {
        ...
        this.ruleForm.recognize == "自动识别"
          ? (language = "")
          : (language = this.ruleForm.recognize);
          ...
          axios
            ...
            .then(
              axios.spread(oneOfReses => {
                ...
                // for (let i of fd1.entries()) {
                //     console.log('%c--> i:', 'color:green', i)
                // }
              })
            )
            .then(res => {
  ...
  mounted() {
    if (this.$route.name === "importList") {
      // 编辑任务不必做修改, 因为importList页没有'编辑'功能
      this.dialogTitle = "导入音频";
    }
  },
  beforeUpdate() {},
  updated() {},
