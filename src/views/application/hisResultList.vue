...
import {getHisResult} from '@/api/history.js'
...
export default {
    ...
    //   taskId: '',
      ...
      columnDefs: [
        ...
        },
        {
          ...
          hide: false, valueGetter: this.platformFormatter
        },
        {
  ...
  computed: {
    ...
    getComResultData(){
      return this.getHistoryDetail().records
    }
  },
  props: {
    taskid: String
  ...
  methods:{
    ...
    ...mapGetters(['getResultData','getNextRecord','getBillStatus','getPlatform','getHistorySwitch','getHistoryId','getHistoryData','getHistoryDetail']),
    exportExcel() {
    ...
    getPageData() {
      ...
      // axios.get(`http://192.168.1.55:9999/api/query?startTime=1531037330&endTime=1563206399&language=&platforms=&threshold=60&minVoiceLength=&maxVoiceLength=&gender=&keywords=&speakerIds=&callLevels=&veracityMark=&numberRules=&limit=${this.pageSize}&start=${(this.currentPage-1)*this.pageSize}&invalidVoice=false&hasAsr=false&hitKeyword=false&hitSpeaker=false&callId=&content=`).then(res => {
      //   this.RESULT_DATA(res.data)
      //   this.loading = false
      // })
    },
    dataInit() {
      ...
      var data = this.getHistoryDetail().records
      // var data = this.getHistoryData().records
      console.error('HisResultList')
      console.log(data)
      for(var k in data){
      ...
      this.rowData = data || []
      this.totalPage = this.getHistoryDetail().total || 0
      this.mytotal = this.getHistoryDetail().total
