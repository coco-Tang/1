<template>
  <div class="importList">
    ...
    <import-video-dialog @setImport="childCreate" :update-msg="updateMsg"  ref="c1"></import-video-dialog>
...
import { //uploadVoicesInfo, getTaskMediaCount,
    getTaskVideos, deleteVideosInfo } from '@/api/impMedia'
...
export default {
  ...
  components: {
    ...
    'import-video-dialog': importVideoDialog,
  ...
  methods:{
    ...
    callDel() {
        ...
        deleteVideosInfo(p, this.getTaskId())
            .then(res => {
    ...
    dataInit() {
        ...
        getTaskVideos(query, this.getTaskId())
            .then(res => {
            ...
            .catch(err => {
                this.loading = false
                console.error('getTaskVideos -- err:', err)
            })


    //   axios.get(`http://localhost:10001/api/testImpList`).then(res=>{
    //     console.log(`http://localhost:10001/api/test1`)
    //     console.log(res)
    //     // this.HISTORY_DATA(res.data)
    //     // this.HIS_SWITCH(false)

    //     let data = res.data.data.records
    //     // data.map((e,i) => e.number = i + 1)
    //     for(let i=0;i<data.length;i++){
    //       console.log(data[i])
    //     //   data[i].appended.importinfo.number = i+1
    //     //   this.rowData.push(data[i].appended.importinfo)
    //       data[i].number = i+1
    ...
    back() {
      this.$router.replace({path: '/importVideo'})
    },
