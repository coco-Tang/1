...
export default {
  ...
  computed: {
    ...
    getComResultData(){
      return this.getMonitorData().records
    }
  },
  props: {
    taskid: String
  ...
  methods:{
    ...
    ...mapGetters(['getMonitorData','getNextRecord','getBillStatus','getPlatform','getHistorySwitch','getAccount']),
    exportExcel() {
    ...
    dataInit() {
      ...
      var data = this.getMonitorData().records
      console.log(data)
      for(var k in data){
      ...
      this.rowData = data || []
      this.totalPage = this.getMonitorData().total || 0

      this.myTotal = this.getMonitorData().total || 0
